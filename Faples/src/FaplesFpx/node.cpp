...
namespace fpl {
	...
	node node::GetNode(node np, std::string path)
	{
		bool gonext = false;
		std::string curr = "";
		std::string next = "";
		...
		else
		{
			...
			curr = path.substr(0, path.find(delimiter));
			path.erase(0, curr.length() + 1);
			next = path;
		...
		throw std::runtime_error("Node doesn't exist for the given path.");
	}
	...
	node::operator bool() const { return m_data ? true : false; }
	int64_t node::get_integer(int64_t def) const {
		if (!m_data) return def;
		switch (m_data->type) {
		case type::none:
		case type::vector:
		case type::bitmap:
	...
	vector2i node::get_vector(vector2i def) const {
		if (m_data && m_data->type == type::vector) return to_vector();
		return def;
	}
	bitmap node::get_bitmap() const {
		if (m_data && m_data->type == type::bitmap && m_file->header->bitmap_count) return to_bitmap();
		return { nullptr, 0, 0 };
	}
	audio node::get_audio() const {
		if (m_data && m_data->type == type::audio && m_file->header->audio_count) return to_audio();
		return { nullptr, 0 };
	}
	...
	int32_t node::x() const { return m_data && m_data->type == type::vector ? m_data->vector[0] : 0; }
	int32_t node::y() const { return m_data && m_data->type == type::vector ? m_data->vector[1] : 0; }
	...
	int64_t node::to_integer() const { return m_data->ireal; }
	double node::to_real() const { return m_data->dreal; }
	...
	audio node::to_audio() const {
		return { reinterpret_cast<char const *>(m_file->base) + m_file->audio_table[m_data->audio.index],
				m_data->audio.length };
	}
	node node::root() const { return { m_file->node_table, m_file }; }
